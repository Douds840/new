...
public class LetterPuzzleGames : MonoBehaviour, IDataPersistence
{
    ...
    public void SaveGameData(ref GameData gameData)
    {
        if (game_index == 3)
        {
            var Signword = gameData.SignWord;
            Signword.score = score;
            Signword.levels[currentlevel - 1].bestTime = this.bestTime;
            Signword.levels[currentlevel - 1].isScored = this.isScored;

            if (isNextlevelUnlocked)
            {
                Signword.levels[currentlevel].isUnlocked = true;



                if (Signword.unlockedlevels < currentlevel + 1 && isAnswered)
                {
                    Signword.unlockedlevels = currentlevel + 1;
                }
